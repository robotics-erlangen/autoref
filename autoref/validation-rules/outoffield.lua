...
function OutOfField.occuring()
	...
	if math.abs(World.Ball.pos.y) > World.Geometry.FieldHeightHalf + World.Ball.radius then
		...
		if ((World.Ball.pos.y > 0 and lastRobot.isYellow)
			...
			OutOfField.message = "(truth) Aimless kick"
			OutOfField.event = Event.aimlessKick(lastRobot.isYellow, lastRobot.id, World.Ball.pos, lastTouchPos)
			return true
		end

		OutOfField.message = "(truth) Ball left field goal line"
		OutOfField.event = Event.ballLeftField(lastRobot.isYellow, lastRobot.id, World.Ball.pos, true)
		return true
	end
	if math.abs(World.Ball.pos.x) > World.Geometry.FieldWidthHalf + World.Ball.radius then
		OutOfField.message = "(truth) Ball left field touch line"
		OutOfField.event = Event.ballLeftField(lastRobot.isYellow, lastRobot.id, World.Ball.pos, false)
		return true
	end
end
